...
import { ESTIMATE, inr } from "@/data/mock";
...
export function ApprovalScreen({ back }: { back?: boolean }) {
  return (
    <Screen>
      <TopBar
        title="Approval"
        back={back}
        right={<Badge label="3 PENDING" tone="purple" />}
      />
      <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 28 }}>
        <Card className="flex-row items-center p-[11px]" style={{ gap: 12 }}>
          ...
          <View className="flex-1">
            ...
            <Txt className="mt-[5px] font-bold text-[14px]">Tata Nexon · Rakesh K.</Txt>
          </View>
        </Card>
        ...
        <Card className="mt-[13px] p-[14px]">
          ...
          <View className="flex-row items-center justify-between border-t border-line pt-[11px]">
            ...
            <Txt className="font-black text-[15px]">{inr(ESTIMATE.total)}</Txt>
          </View>
        </Card>
